...
public class MenuItem {
    ...
    public static void saveNewItem(int menuid, int sectionid, MenuItem mi, int pos) {
        String itemInsert = "INSERT INTOMenuItems (menu_id, section_id, description, recipe_id, position) VALUES (" +
                menuid +
                ", " +
                sectionid +
                ", " +
    ...
    public static ObservableList<MenuItem> loadItemsFor(int menu_id) {
        ObservableList<MenuItem> result = FXCollections.observableArrayList();
        ArrayList<Integer> recids = new ArrayList<>();
        String query = "SELECT * FROM MenuItems WHERE menu_id = " + menu_id +" ORDER BY position";
        PersistenceManager.executeQuery(query, new ResultHandler() {
            @Override
            public void handle(ResultSet rs) throws SQLException {
                MenuItem mi = new MenuItem();
