...
public class Menu {
    ...
    private static Map<String, Menu> loadedMenuTitles = FXCollections.observableHashMap();
    private static Map<Menu, Boolean> loadedMenusMap = FXCollections.observableHashMap();
    private static Map<Menu, Boolean> loadedProposedMenusMap = FXCollections.observableHashMap();
    private int id;
    ...
    public static Menu getMenuFromId(int approved_menu) {
        Menu m = new Menu();
        if(loadedMenus.containsKey(approved_menu)) {
            m = loadedMenus.get(approved_menu);
        }
        return m;
    }

    public static Menu getMenuByTitle(String s) {
        Menu m = new Menu();
        if(loadedMenuTitles.containsKey(s)){
            m = loadedMenuTitles.get(s);
        }
        return m;
    }
    ...
    public static ObservableList<Menu> loadAllMenus() {
        ...
        for (int i = 0; i < newMenus.size(); i++) {
            ...
            // find if "in use"
            String inuseQ = "SELECT * FROM Services WHERE approved_menu = " + m.id;
        ...
        for (Menu m: newMenus) {
            loadedMenus.put(m.id, m);
            loadedMenuTitles.put(m.getTitle(),m);
        }
        return FXCollections.observableArrayList(loadedMenus.values());
    }

    public static Map<Menu,Boolean> loadAllMenusMap() {
        String query = "SELECT * FROM Menus WHERE 1";
        ...
        PersistenceManager.executeQuery(query, new ResultHandler() {
            ...
            public void handle(ResultSet rs) throws SQLException {
                int id = rs.getInt("id");
                if(!loadedMenusMap.containsKey(id)) {
                    Menu m = new Menu();
                    m.id = id;
                    m.title = rs.getString("title");
                    menus.add(m);
        ...
        for (Menu m: menus) {
            if(currentScheda.getChef() != null)
                loadedMenusMap.put(m, m.proposed);
            else loadedMenusMap.put(m, false);
        }

        return loadedMenusMap;
    }
